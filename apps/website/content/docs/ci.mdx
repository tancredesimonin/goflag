...
seo:
  ...
  script:
    ...
    - pnpm build
    - >
      pnpm dlx @goflag/cli@__CLI_VERSION__ http://localhost:3000
...
seo:external:
  ...
  script:
    - pnpm dlx @goflag/cli@__CLI_VERSION__ https://example.com --fail-on never --json > links.json
```
...
```bash
pnpm dlx @goflag/cli@__CLI_VERSION__ …
```
...
| `--baseline` + `--regressions-only` | No _new_ findings at or above `--fail-on`, and the total is within `--max-debt` | A new finding at or above `--fail-on`, or `--max-debt` exceeded |
...
green. The full table is in the [CLI reference](/docs/cli#exit-codes).
