...
server. It makes the job faster and rules out any browser involvement, but a
single client-rendered route makes its findings misleading — and sites drift,
one route at a time, without anything warning you. See
...
that touched nothing. Bump it deliberately, read the [changelog](/changelog), and
...
| Code | Meaning                                                                                                                  |
| ---- | ------------------------------------------------------------------------------------------------------------------------ |
| `0`  | No findings at or above `--fail-on`. Also returned by `--update-baseline`, `--help` and `--version`.                     |
| `1`  | Findings at or above `--fail-on`, a new regression against a baseline, or `--max-debt` exceeded. This is the gate.       |
| `2`  | The run itself failed — bad arguments, the entry URL unreachable, `--start` never came up. Not a verdict about the site. |

Treat `2` as a broken job, not as a red site. It means goflag never got to
judge anything.
