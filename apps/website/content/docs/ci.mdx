...
## Why `--static --no-external`

`--static` needs no browser, so the job runs on a plain Node image and cannot be
slowed — or broken — by a Chromium download. On a server-rendered site it changes
no finding.
...
seo:external:
  rule: schedule
  script:
    - npx --yes "@goflag/cli@0.1.4" https://example.com --static --fail-on never --json > links.json
```
