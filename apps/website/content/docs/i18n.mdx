...
the count is reported under `diagnostics.declaredClusters`. A declaration decides
**identity only**: it never fills a cell and never asserts that a page exists.
...
added, and every baseline would go red on a site where nothing changed. Without
a sitemap declaration the hole count on a slug-translating site is not
meaningful: rely on the reciprocity findings and the two site-wide rules
instead, and use `--ignore-holes` for the affected route families.
...
that **does** have a declared axis a route like `/cv` still becomes a column and
still reports holes. Add it to `--ignore-holes`, or check that its pages declare
`<html lang>` — that cross-check is what the candidate report uses to tell a CV
page from a Chuvash edition.
