...
[`hreflang.sitemap-mismatch`](/docs/rules/hreflang.sitemap-mismatch) reads the
same declarations, so a declared cluster produces neither a hole nor a mismatch:
the rule groups both the sitemap's entries and the crawled pages by cluster
before comparing them, which is what stops `/en/pricing` and `/fr/tarifs` from
looking like two half-covered routes.

Without a declaration there is nothing to pair on, and both fall back to the
pathname. On a slug-translating site that declares none, the hole count is not
meaningful and neither is that rule — it reports the `<head>` and the sitemap as
disagreeing on exactly the pages that are correct. The trustworthy findings there
are the reciprocity codes below and
