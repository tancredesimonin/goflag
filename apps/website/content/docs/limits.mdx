...
cluster with `xhtml:link` in your sitemap fixes it, for the matrix and for
[`hreflang.sitemap-mismatch`](/docs/rules/hreflang.sitemap-mismatch) alike; a
site that translates its slugs and declares nothing gets one hole per locale on
pairs that are fully translated, and a mismatch warning on each half. See
[Translations](/docs/i18n#the-matrix).
...
in [Translations](/docs/i18n#the-bug-that-started-this).

They are published rather than buried because a tool that reports defects has to
be auditable itself, and because the second one cost somebody an afternoon.
