...
missing: the escalation is skipped, the page is judged as served, and **nothing
in the report says so** — no warning, no diagnostics field. On an SPA that is a
full column of phantom `title.missing` / `description.missing` findings. Install
Chromium before you believe them.

**Translations are paired by pathname.** A route is the URL with its locale
segment removed, so goflag only recognises a pair when the slug is identical in
every locale. A site that translates its slugs — `/en/pricing` and `/fr/tarifs` —
produces two routes, each filled in one locale, and therefore two holes, however
correct its `hreflang` is. See [Translations](/docs/i18n#the-matrix).
