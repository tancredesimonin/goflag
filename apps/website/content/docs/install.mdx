...
Without it, goflag does not fail: the page is judged on its static HTML, and
nothing in the report says so — the skipped re-render is silent today. If a
page's findings read as "missing everything" and the metadata is really filled in
the browser, that is the symptom. Install Playwright and Chromium, re-run, and
compare.
